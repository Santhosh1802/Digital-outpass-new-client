...
export default function AdminManageSecurity({ toast }) {
  ...
  const [newSecurity, setNewSecurity] = useState({ name: "", email: "", primary_number: "", password: "" });
  ...
  const handleSubmit = async() => {
    ...
    if(res.error===""){
      toast.current.show({severity:"success",summary:"Success",detail:"Security added"});
    }
    if(res.message===""){
      toast.current.show({severity:"error",summary:"Error",detail:"Error adding security"});
    }
  ...
  const handleDelete = (rowData) => {
    confirmDialog({
      ...
      accept: async () => {
        ...
        setFilteredSecurity(filteredSecurity.filter((item) => item.id !== rowData.id));
      },
    });
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center" }}>
      ...
      <div style={{ marginTop: "4em" }}>
        ...
        <Button label="Add Security" onClick={handleAddSecurity} style={{ margin: "1em" }} />
      ...
      <Dialog visible={modalVisible} onHide={() => setModalVisible(false)} header="Add Security">
        <div style={{display:"flex",flexDirection:"column",justifyContent:"space-between",width:"500px",height:"70vh"}}> 
        ...
        <InputText placeholder="Name" value={newSecurity.name} onChange={(e) => setNewSecurity({ ...newSecurity, name: e.target.value })} />
        <InputText placeholder="Email" value={newSecurity.email} onChange={(e) => setNewSecurity({ ...newSecurity, email: e.target.value })} />
        <InputText placeholder="Phone" value={newSecurity.primary_number} onChange={(e) => setNewSecurity({ ...newSecurity, primary_number: e.target.value })} />
        <InputText placeholder="Password" type="password" value={newSecurity.password} onChange={(e) => setNewSecurity({ ...newSecurity, password: e.target.value })} />
        <Button label="Submit" onClick={handleSubmit} style={{ marginTop: "1em" }} />
        </div>
      </Dialog>
    </div>
  );
